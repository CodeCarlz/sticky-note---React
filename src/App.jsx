...
function App() {
  ...
  const handleEdit = (id) => {
    const editeList = items.map((item) =>
  ...
  return (
    <main>
      <form className="topContainer" onSubmit={handleAddItems}>
        ...
        />
        <FaRegSquarePlus id="saveIcon" type="submit" onClick={handleAddItems} />
      </form>

      <ul
        className="middleContainer"
        style={{
          gridTemplateColumns: `${items.length <= 1 ? "repeat(4, 500px)" : ""}`,
        }}
      >
        {items.map((item) => (
          <li className="item" key={item.id}>
            <p
              ...
              style={{
                backgroundColor: `${item.editable ? "rgb(255, 220, 160)" : ""}`,
              }}
              onBlur={(e) =>
                handleEditContentChange(item.id, e.target.innerText)
              }
            >
              {item.item}
            </p>
